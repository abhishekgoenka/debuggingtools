...
namespace BuildAndDeploy.SeleniumHelpers
{
    static class WebDriverExtensions
    {
        public static IWebElement FindElementByJQuery(this IWebDriver driver, string selector)
        {
            ...
            var enumerable = elements as IList<object> ?? elements.ToList();
            if (!enumerable.Any())
            {
                // retry with a delay 
                Thread.Sleep(4000);
            ...
            if (enumerable.Count() > 1)
                throw new InvalidOperationException(
                    $"Multiple elements found with selector {formattedSelector}. Make sure that the selector uniquely identifies a single element.");
            return enumerable.FirstOrDefault() as IWebElement;
        }
