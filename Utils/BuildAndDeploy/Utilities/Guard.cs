...
namespace BuildAndDeploy.Utilities
{
    static class Guard
    {
        [DebuggerStepThrough]
        ...
        public static void IsNotNullOrEmpty<TExpection, TValue>(Nullable<TValue> value, string format, params object[] parameters)
            ...
            where TValue : struct
        {
            if (value != null && (value.HasValue || value.Value.Equals(default(TValue))))
            {
                throw GetException<TExpection>(format, parameters);
            }
        }
        ...
        [DebuggerStepThrough]
        public static void IsNotNullOrEmpty<TValue>(TValue? value, string argumentName) where TValue : struct
        {
